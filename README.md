...
1. 训练前务必确定`JPEGImages`和`TemplateImages`文件夹下的同名图片，方向相同。
   怀疑PIL读如的图片方向和在文件夹看到的图像方向不一致，建议使用`Temp.py`文件一个一个测试一下。
   针对以上猜测，在读入图片之后，使用`image = ImageOps.exif_transpose(image)`修正图片方向，以免出现图片方向与标签不一致的情况。（该问题有待验证）
2. 由于图片较少，应该按照warning中建议的epoch数设置，这里7张图片，设置为4000+应该够了。
3. 训练图片非常少，故该仓库仅供测试搭建的孪生网络是否好使，故这里是极其过拟合的，不能实际运用。
4. 训练好的模型链接如下，直接读取预测即可：
   链接: https://pan.baidu.com/s/1g420090zsoESFmmyflOf8Q 提取码: 1etq 复制这段内容后打开百度网盘手机App，操作更方便哦
